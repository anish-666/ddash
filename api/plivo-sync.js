...
async function listRecordingsForCall({ authId, token, callUuid }) {
  ...
  // Global recordings (recent), then filter
...
function pickFromCall(c) {
  ...
  const dur =
    ...
    null;
  // sometimes CDR already has a recording URL
  const cdr_rec_url = c?.recording_url || c?.url || null;
...
module.exports.handler = async (event) => {
  ...
  try {
    requireAuth(event); // allows DISABLE_AUTH=1 or X-Admin-Key
    ...
    for (let page = 0; page < 10; page++) {
      ...
      for (const c of list.items) {
        ...
        // Prefer recording from recordings API, fallback to CDR-level recording URL if any
        const finalRecUrl = recInfo.recording_url || norm.cdr_recording_url || null;
        const finalDur = norm.duration_sec || recInfo.recording_duration_sec || null;

        await query(`
          INSERT INTO docvai_calls
            (provider_call_id, to_number, from_number, status, duration_sec, recording_url, transcript_text, transcript_url, direction, payload, created_at)
          VALUES
            ($1, $2, $3, $4, $5, $6, NULL, NULL, $7, $8::jsonb, NOW())
          ON CONFLICT (provider_call_id) DO UPDATE SET
            ...
            direction     = COALESCE(EXCLUDED.direction,     docvai_calls.direction),
