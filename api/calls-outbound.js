...
module.exports.handler = async (event) => {
  ...
  // Only POST is allowed for creating outbound calls
  if (method !== 'POST') {
    return {
      statusCode: 405,
  ...
  try {
    // Enforce auth (respects DISABLE_AUTH=1 bypass)
    requireAuth(event);
    ...
    if (numbers.length === 0) {
      return {
        statusCode: 400,
        headers: corsHeaders(event),
        body: JSON.stringify({ error: 'numbers_required' })
      };
    }
    ...
    // Caller ID (trial / compliance usually requires it)
    const FROM_NUMBER =
      fromNumberOverride ||
    ...
    if (!BOLNA_API_KEY) {
      return {
        statusCode: 500,
        headers: corsHeaders(event),
        body: JSON.stringify({ error: 'missing_BOLNA_API_KEY' })
      };
    }
    ...
    if (!AGENT) {
      return {
        statusCode: 400,
        headers: corsHeaders(event),
        body: JSON.stringify({ error: 'agent_id_required' })
      };
    }

    // --- Call fan-out with controlled parallelism ---
    ...
    async function one(num) {
      ...
      try {
        ...
        // ignore JSON parse errors; keep raw
      }

      const ok = res.ok;
      const id =
        (data && (data.id || data.call_id || data.execution_id)) || null;
